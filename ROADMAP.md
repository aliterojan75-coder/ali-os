...
> **تصمیم دامنه‌ی فاز ۴ ثبت شد — پایین همین سند، بخش «بک‌لاگ تأیید‌شده‌ی فاز ۴».**
> توسعه‌های پیشنهادی روی مدل ماهانه‌ی Financial طبق نظر علی انجام **نمی‌شوند**.
> **وضعیت فاز ۴ (۲۰۲۶-۰۸-۲۷، دوم):** P0 ✅ · P1 ✅ کد (تنظیم ۲ secret با علی) · P2 ✅ · P3 ✅ ·
> زیرساخت P5 ✅ · کش گوگل + gzip + رفع دوباره‌نویسی JS داشبورد ✅ — ۱۵۸ تست سبز.
> **یاد مهم:** فیچرهای فاز ۲+۳ از PR #4 روی `main` merge شد؛ فیچرهای فاز ۴ (این کامیت‌ها)
> در PR جدید است تا مرج نشود، در اپلیکیشن دیده نمی‌شود.
...
> ✅ هر سه انجام شد: پیشنهاد موضوع از GSC و Monitoring کم‌مصرف در همین PR؛ ارسال پیگیری sales
> به تلگرام مخاطب در فاز ۴ (P2) پیاده شد.
...
## بک‌لاگ تأیید‌شده‌ی فاز ۴ (سنگ‌تموم — ۲۰۲۶-۰۸-۲۷، انتخاب شخصی علی)

> **پیشرفت اجرا (همان روز):** P0/P2/P3 تمام + P1 در حد کد (کار کاربر: افزودن secretها) +
> P5 زیرساختی + بخش سرعت. P4، P6..P11 در صف بعدی‌اند. باگ بحرانی کشف‌شده و رفع‌شده:
> بلوک JS داشبورد در PR #4 دو بار کپی شده بود و `let` تکراری کل اسکریپت را با SyntaxError
> می‌کشت — حذف شد و `node --check` سبز است (اقدام بهداشتی بعدی: dedupe ناحیهٔ HTML/CSS هم باقی است).

> ترتیب = اولویت اجرا. همه‌ی عملیات خارجی **فقط** از دروازه‌ی تأیید §19.
> پایه‌ی اجرا: PR #4 (فاز ۲+۳) → مرج در main، سپس این موارد کامیت‌به‌کامیت.

### P0 — امنیت webhook (قبل از هر contact خارجی، حیاتی)
- الان `/webhook` فقط `X-Telegram-Bot-Api-Secret-Token` را چک می‌کند که **فرستنده تلگرام است**؛
  نه اینکه فرستنده **علی است**. هر غریبه‌ای بات را پیدا کند → `/dossier` `/crm` `/finance` می‌خواند.
- `ADMIN_CHAT_ID` در config/env؛ چت‌های ناشناس: پاسخ مؤدبانه بدون داده + لاگ `event` امنیتی.
- Whitelist دوم برای **پاسخ کارفرماها** (فقط `chat_id`های ثبت‌شده در `crm_contacts.telegram_chat_id`)
  که فقط اجازه‌ی *گفتگو در جریان یادآور/پیگیری* دارند، نه هیچ دستور داده‌ای — پیش‌نیاز P7.
- تست: ناشناس → 403-منطقی/متن خنثی، مالک → روتینگ فعلی، کارفرما → فقط inbox.

### P1 — بکاپ شبانه Turso
- GitHub Action شبانه: libsql `sync`/dump → ریپوی private؛ `docs/BACKUP.md` + restore.
- داده CRM/مالی = دارایی؛ Render رایگان هیچ تضمینی نمی‌دهد.

### P2 — Sales Agent → ارسال مستقیم به تلگرام مخاطب (گپ باقی‌مانده‌ی PR #4)
- عین الگوی `financial.send_reminder`: `sales.send_followup` با ریسک 🟡 + `client_telegram_chat_id`
  + پیش‌نمایش `dry_run` + لاگ در `crm_interactions` بعد از ارسال.
- intent «پیگیری بده» در Master + دکمه 🟡 در شیت دیلِ داشبورد.

### P3 — آشکارساز صفحات نزولی (طلای خوابیده‌ی `gsc_daily_stats`)
- `app/integrations/gsc_storage.py`: مقایسه ۲۸ روز اخیر vs ۲۸ روز قبل → افت >۳۰٪ کلیک/نمایش.
- خروجی: notification (🔴 مهم) + کارت «صفحات در حال مرگ» در تب SEO/خلاصه + دکمهٔ
  «بازنویسی سئو» که `rewrite_for_seo` را روی همان URL اجرا می‌کند.

### P4 — FAQ → Schema JSON-LD خودکار
- هنگام publish (executor وردپرس) از FAQ پیش‌نویس، `FAQPage` JSON-LD ساخته و در فیلد
  Rank Math schema قرار می‌گیرد؛ `seo.audit` هم آیتم «schema» به اسکور اضافه کند.

### P5 — کانیبالیزاسیون واقعی از GSC
- دو صفحه روی یک کوئری (query→page دو‌تایی در ۲۸ روز اخیر) → پیشنهاد ادغام/redirect در
  `content.cannibalization`؛ فعلی (Jaccard روی عنوان) به‌عنوان fallback می‌ماند.

### P6 — گزارش ماهانه مشتری (ابزار تمدید قرارداد ⭐)
- `GET /api/clients/<contact>/report?month=1404-06` → HTML فارسی RTL (SVG inlined، بدون CDN):
  کلیک/نمایش/جایگاه از `gsc_daily_stats` + مقالات منتشرشده + تسک‌های انجام‌شده + واریزی همان ماه.
- از دروازه 🟡 `client.send_report` → تلگرام/ایمیل کارفرما با برند «دستیار آژانس نت نوا».

### P7 — خواندن پاسخ کارفرما (inbox)
- پیام دریافتی از چتِ whitelist‌شده‌ی کارفرما → طبقه‌بندی LLM
  (`paid` / `will_pay_date` / `question` / `other`) → ثبت `crm_interactions` + نوتیف به علی؛
  `paid` → پیشنهاد 🟢 ثبت پرداخت. (وابسته به P0)

### P8 — تایم‌شیت + ظرفیت + «یک کار امروز»
- جداول `time_logs(task_id, started_at, ended_at)`؛ دکمه ▶️/⏹ روی کارت تسک در تلگرام + داشبورد؛
  گزارش «نرخ ساعتی واقعی ÷ مبلغ قرارداد ماهانه» در پرونده پروژه (نمایشی، بدون دست‌زدن به مدل مالی).
- گزارش صبحگاهی: نوار ظرفیت ۷۲ ساعت ماهانه + بخش «🎯 امروز فقط این» (تاپ‌استریم PM Agent).

### P9 — PWA آفلاین Mini App
- service worker: کش شل + آخرین پاسخ `/api/analytics` در localStorage → «حالت آفلاین» با
  برچسب زمان داده؛ ادامه فلسفه non-blocking (فونت/SDK/SVG).

### P10 — تقویم محتوایی فصلی (جلالی)
- جدول `content_calendar(project_id, month_jalali, topic, source, status)`؛ پیشنهاد خودکار
  نوروز/یلدا/مهرگان × محصولات metadata؛ cron ماهانه: صف draft (انتشار 🔴 دست علی).

### P11 — تصویر واقعی مقاله (اختیاری — نیازمند Image Provider)
- `IMAGE_API_*` در config؛ `image_prompt` → تولید → `wp media/upload` → featured image،
  همه از دروازه 🟡 `content.attach_image`. بدون provider، وضعیت فعلی (ذخیره prompt) می‌ماند.

## ❌ ردشده‌ها — ثبت تصمیم (انجام نشود، در سشن‌های بعدی هم احیا نشود)

توسعه‌های پیشنهادی روی **مدل ماهانه Financial** — طبق نظر علی (۲۰۲۶-۰۸-۲۷):
1. ساخت خودکار رکورد ماه بعد (recurring) — لازم نیست؛ دستی/با کمک AI کافی است
2. سود خالص پروژه (واریزی − هزینه) و رتبه‌بندی — خارج از نیاز
3. پارس عکس فیش/SMS بانک برای تطبیق واریز — خارج از نیاز
4. نمره خوش‌حسابی کارفرما از data پرداخت — خارج از نیاز
5. پیوست صورتحساب به پیام یادآور — خارج از نیاز

> مدل فعلی (`project_incomes` ماهانه + یادآور خودکار 🟡) **همان‌طور که در PR #4 است** می‌ماند؛
> هیچ تغییری روی ساختار مالی اعمال نشود.
